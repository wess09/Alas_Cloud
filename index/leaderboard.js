...
async function updateProfile() {
    ...
    if (!deviceId) {
        msgEl.innerText = "⚠️ Device ID is required.";
        msgEl.style.color = "#f87171";
        return;
    }
    if (!username) {
        msgEl.innerText = "⚠️ Username cannot be empty.";
        msgEl.style.color = "#f87171";
        return;
    }
    ...
    try {
        ...
        if (res.ok) {
            msgEl.innerText = "✅ Profile updated! Refreshing leaderboard...";
            ...
            msgEl.innerText = "❌ Error: " + (data.error || "Unknown error");
            msgEl.style.color = "#f87171";
        }
    } catch (e) {
...
async function loadLeaderboard(page) {
    ...
    tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; opacity:0.7;">Loading...</td></tr>';
    ...
    try {
        ...
        if (!data.data || data.data.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; opacity:0.5;">No data available</td></tr>';
            return;
        }
        ...
        data.data.forEach((entry, index) => {
            ...
            // Highlight current user if device ID matches
            const myDeviceId = localStorage.getItem('alas_device_id');
            const isMe = myDeviceId === entry.device_id;
            ...
            html += `
                <tr ${rowClass}>
                    ...
                    <td>
                        <span style="font-weight:600;">${entry.username || 'Unknown Commander'}</span>
        ...
        console.error(e);
        tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; color:#f87171;">Failed to load leaderboard</td></tr>';
    }
}
