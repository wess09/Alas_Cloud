...
const forceUpdateConfigKey = "force_update"
...
// GetForceUpdateStatus 获取当前强制更新状态（公开）。
func GetForceUpdateStatus(c *gin.Context) {
	var config models.SystemConfig
	result := database.DB.Where("key = ?", forceUpdateConfigKey).First(&config)
	if result.Error != nil {
		c.JSON(http.StatusOK, false)
		return
	}
...
// AdminGetForceUpdateStatus 获取当前强制更新状态（管理员）。
func AdminGetForceUpdateStatus(c *gin.Context) {
	var config models.SystemConfig
	result := database.DB.Where("key = ?", forceUpdateConfigKey).First(&config)
	if result.Error != nil {
		c.JSON(http.StatusOK, gin.H{"force_update": false})
		return
	}

	c.JSON(http.StatusOK, gin.H{"force_update": config.Value == "true"})
}

// AdminToggleForceUpdate 切换强制更新状态（管理员）。
func AdminToggleForceUpdate(c *gin.Context) {
	isActive := c.Query("is_active") == "true"
	val := "false"
	if isActive {
		val = "true"
